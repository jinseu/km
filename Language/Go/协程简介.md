...
4. _Gsyscall means this goroutine is executing a system call. It is not executing user code. The stack is owned by this goroutine. It is not on a run queue. It is assigned an M.
...
Go 语言中，包括用户入口函数`main.main`在内的所有goroutine，都是通过`runtime.newproc1` 这个函数创建的（`runtime.newproc`实现时也是调用了`runtime.newproc1`），go 关键字最终也会被翻译为对`runtime.newproc`的调用。
