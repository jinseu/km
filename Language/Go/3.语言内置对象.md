...
创建slice的逻辑也相对简单，根据cap以及slice类型，计算要分配的内存大小，然后调用`mallocgc`分配一段内存，并将array指向这段内存。

比较复杂的点在于调用`append`，向slice中增加元素，此处的具体实现分为两步分

1. 调用内建函数`append`具体实现在`src/cmd/compile/internal/gc/ssa.go`
2. 由`append`调用 `growslice` 函数实现slice的扩张。
