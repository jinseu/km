...
only works for new-style classes，通过`__mro__`中的顺序，在其中查找对应的方法。

##### `__new__(cls[, ...])`
如果override了`__new__`方法，那么`__init__`方法将不会被调用。第一个参数是对应的类对象。
必须返回一个值，负责创建的对象将为None。
...
每一个python对应都有一个__dict__属性，里面保存了该对象绑定的所有属性。`__slots__`属性的值为一个元组，在定义该属性之后，对于该类的对象，就只能绑定在元组中定义的属性。
...
python在创建对象是本质上先调用了`type(name, bases, dict)`，name 是类名，将变成`__name__`属性，bases则是基类元组，dict则是class的namespace，将变成`__dict__`属性。然后在`__init__`函数中绑定属性。
...
For an old-style class, the statement x.__class__ provides the class of x, but type(x) is always <type 'instance'>

new-style type(x) is typically the same as x.__class__，需要注意的是，此处也是不一定的，允许override x.__class__。
